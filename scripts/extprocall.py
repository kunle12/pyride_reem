...
class ProcConduit:
  ...
  def setToMannequinMode( self, isYes ):
    if not isinstance( isYes, bool ):
      ...
      if self.mannequin:
        ...
        PyREEM.say( "Start mannequin mode." )
    else:
      if self.mannequin:
        ...
        PyREEM.say( "Stop mannequin mode." )
  ...
  def startJoystickControl( self ):
    ...
    PyREEM.say( "Start joystick control." )

  def stopJoystickControl( self ):
    if self.joyControl:
      self.killProc( self.joyControl )
      self.joyControl = None
      PyREEM.say( "Stopped joystick control." )

  def setCallbacks( self ):
    PyREEM.setProjectorOff = self.setProjectorOff
    PyREEM.setToMannequinMode = self.setToMannequinMode
    PyREEM.startDataRecording = self.startDataRecording
    PyREEM.stopDataRecording = self.stopDataRecording
    PyREEM.startJoystickControl = self.startJoystickControl
    PyREEM.stopJoystickControl = self.stopJoystickControl
